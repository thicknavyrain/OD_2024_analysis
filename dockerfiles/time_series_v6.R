...
# Fit the zero-inflated negative binomial mixed-effects model using glmmTMB with increased iterations
model_fit <- glmmTMB(formula, 
                     data = filtered_data, 
                     ziformula = ~1,         # Zero-inflation formula
                     family = nbinom2(),     # Negative binomial family
                     control = glmmTMBControl(
                        optCtrl = list(iter.max = 100, eval.max = 100)  # Increase the maximum number of iterations
                     ),
